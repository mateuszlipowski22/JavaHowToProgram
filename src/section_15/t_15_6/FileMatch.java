...
public class FileMatch {
    public void processFiles(String oldMast, String transactions, String newMast, String error) {
        ...
        try(BufferedReader input = Files.newBufferedReader(Paths.get(oldMast))) {
            Map<Integer, Account> finalAccountList = accountList;
            JAXB.unmarshal(input, Accounts.class).getAccounts().forEach(account -> {
                finalAccountList.put(account.getAccountNumber(),account);
            });
        }catch (IOException e) {
