package section_17.employee.e_17_21;
...
public class ProcessingEmployees {
    public static void main(String[] args) {
        ...
        employeeCountByDepartment.forEach((department,count)->{
            System.out.printf("%s ma %d pracowników%n",department,count);
        });

        System.out.printf("%nSuma zarobków pracowników (za pomocą metody sum): %.2f",
                list.stream()
                        .mapToDouble(Employee::getSalary)
                        .sum());

        System.out.printf("%nSuma zarobków pracowników (za pomocą metody reduce): %.2f",
                list.stream()
                        .mapToDouble(Employee::getSalary)
                        .reduce(0,(x,y)->x+y));

        System.out.printf("%nŚrednia zarobków pracowników: %.2f",
                list.stream()
                        .mapToDouble(Employee::getSalary)
                        .average()
                        .getAsDouble());
